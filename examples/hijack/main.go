...
func main() {
	...
	// === 示例 1：LoadResponse + Cookie 注入 ===
	fmt.Println("=== 示例 1: LoadResponse + Cookie ===")
...
func demoLoadResponse(ctx context.Context, browser *obscura.Browser) {
	...
	router.Add("*", "", func(ctx context.Context, req *obscura.HijackRequest, res *obscura.HijackResponse) {
		...
		// Set-Cookie 已由 hijack 内部自动注入，仅打印日志
		if len(res.SetCookieHeaders) > 0 {
			fmt.Printf("  [Cookie] 收集 %d 个 Cookie（自动注入）\n", len(res.SetCookieHeaders))
			fmt.Printf("  [Cookies] %v\n", res.SetCookieHeaders)
		}
